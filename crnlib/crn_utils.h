...
static inline bool is_bit_set(uint bits, uint mask) {
  return (bits & mask) != 0;
}

static inline void set_bit(uint& bits, uint mask, bool state) {
  if (state)
    bits |= mask;
  else
    bits &= ~mask;
...
static inline bool is_flag_set(uint bits, uint flag) {
  CRNLIB_ASSERT(flag < 32U);
  return is_bit_set(bits, 1U << flag);
}

static inline void set_flag(uint& bits, uint flag, bool state) {
  CRNLIB_ASSERT(flag < 32U);
  set_bit(bits, 1U << flag, state);
}

static inline void invert_buf(void* pBuf, uint size) {
...
static inline bool write_val(uint8 val, void*& pBuf, uint& buf_size, bool buffer_is_little_endian) {
  return write_obj(val, pBuf, buf_size, buffer_is_little_endian);
}
static inline bool write_val(uint16 val, void*& pBuf, uint& buf_size, bool buffer_is_little_endian) {
  return write_obj(val, pBuf, buf_size, buffer_is_little_endian);
}
static inline bool write_val(uint val, void*& pBuf, uint& buf_size, bool buffer_is_little_endian) {
  return write_obj(val, pBuf, buf_size, buffer_is_little_endian);
}
static inline bool write_val(int val, void*& pBuf, uint& buf_size, bool buffer_is_little_endian) {
  return write_obj(val, pBuf, buf_size, buffer_is_little_endian);
}
static inline bool write_val(uint64 val, void*& pBuf, uint& buf_size, bool buffer_is_little_endian) {
  return write_obj(val, pBuf, buf_size, buffer_is_little_endian);
}
static inline bool write_val(float val, void*& pBuf, uint& buf_size, bool buffer_is_little_endian) {
  return write_obj(val, pBuf, buf_size, buffer_is_little_endian);
}
static inline bool write_val(double val, void*& pBuf, uint& buf_size, bool buffer_is_little_endian) {
  return write_obj(val, pBuf, buf_size, buffer_is_little_endian);
}
...
static inline void endian_swap_mem16(uint16* p, uint n) {
  while (n--) {
    *p = swap16(*p);
    ++p;
  }
}
static inline void endian_swap_mem32(uint32* p, uint n) {
  while (n--) {
    *p = swap32(*p);
    ++p;
  }
}
static inline void endian_swap_mem64(uint64* p, uint n) {
  while (n--) {
    *p = swap64(*p);
    ++p;
  }
...
static inline void endian_swap_mem(void* p, uint size_in_bytes, uint type_size) {
  switch (type_size) {
    case sizeof(uint16):
...
static inline void fast_memset(void* pDst, int val, size_t size) {
  memset(pDst, val, size);
}

static inline void fast_memcpy(void* pDst, const void* pSrc, size_t size) {
  memcpy(pDst, pSrc, size);
}

static inline uint count_leading_zeros(uint v) {
  uint temp;
  uint n = 32;
...
static inline uint count_leading_zeros16(uint v) {
